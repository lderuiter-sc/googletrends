...
# Reduced keywords for leaderboard (faster API calls)
KEYWORDS = KEYWORDS_PRIMARY  # Use only 2 keywords for speed

# Additional broader keywords (saved for future use)
...
def fetch_fresh_data():
    ...
    try:
        ...
        for i, keyword in enumerate(KEYWORDS):
            ...
            # Add delay between requests (reduced for speed)
            if i > 0:
                delay = random.uniform(3, 5)  # Faster delays
                print(f"Waiting {delay:.1f} seconds...")
                time.sleep(delay)
            
            try:
                pytrends.build_payload([keyword], timeframe='today 1-m')  # Shorter timeframe for speed
...
def fetch_timeseries_data():
    ...
    try:
        ...
        # Map display names to Google Trends geo codes (5 countries: FR, BE, AE, GB, US)
        country_geo_map = {
            'France': 'FR',
            'Belgium': 'BE', 
        ...
        for country in TRACKED_COUNTRIES:
            ...
            try:
                ...
                # Add delay between countries (adjusted for 6 countries)
...
def get_fallback_timeseries():
    """Return fallback MONTHLY time series data for 5 countries (Jan 2025 to current month)"""
    ...
    # Generate sample data for each month (increasing trend) for 5 countries
...
def refresh_timeseries():
    """Force refresh monthly time series data for 5 countries"""
    global cached_timeseries, timeseries_cache_timestamp
    
    print("Force monthly time series refresh requested (5 countries)")
    fresh_data = fetch_timeseries_data()
